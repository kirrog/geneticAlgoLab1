...
if __name__ == "__main__":
    with open("results.tsv", "r") as f:
        ...
        for up_ in ["01000", "10000"]:
            ...
            plt.legend()
            plt.title(f"Plot time def up lvl {up_}")
            plt.savefig(f"Plot time def up lvl {up_}")
            # plt.show()
            plt.close()
        for count_ in ["01000", "10000"]:
            ...
            plt.xlabel("k")
            plt.ylabel("Average time")
            plt.title(f"Plot time def count {count_}")
            plt.savefig(f"Plot time def count {count_}")
            # plt.show()
            plt.close()
        ...
        for up_ in ["01000", "10000"]:
            ...
            plt.xlabel("n")
            plt.ylabel("Average time")
            plt.title(f"Plot time opt up lvl {up_}")
            plt.savefig(f"Plot time opt up lvl {up_}")
            # plt.show()
            plt.close()
        for count_ in ["01000", "10000"]:
            x = [x[0] for x in data if x[1] == count_]
            ...
            plt.xlabel("k")
            plt.ylabel("Average time")
            plt.title(f"Plot time opt count {count_}")
            plt.savefig(f"Plot time opt count {count_}")
            # plt.show()
            plt.close()
